...
def dijkstra(graph, start_node, end_node):
    ...
    while distance_dict:
        current_node, node_distance = sorted(distance_dict.items(),
                                             key=lambda x: x[1])[0]
        ...
        for edge in current_node.edges:
            if edge.node in distance_dict:
                distance_to_neighbor = edge.distance + node_distance
...
print('Shortest Distance from {} to {} is {}'
      .format(node_u.value, node_y.value, dijkstra(graph, node_u, node_y)))
